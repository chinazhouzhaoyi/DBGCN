...
Please specify the hyperparameters before executing the [build_sparse_labeling.py],  
...
and automatically propagate the sparse labels to all points in the point cloud; finnaly this step conducts label accuracy evaluation.  
Please specify the hyperparameters before executing the [DBGCN_main.py]  
There are several parameters need to be determined by the user for tuning the best model performance, 
...
## Citation
D. Li, Z. Zhou, S. Yang, and W. Wen, “DBGCN: Dual-branch Graph Convolutional Network for organ instance inference on sparsely labeled 3D plant data,” Plant Phenomics, vol. 8, no. 2, June 2026, 100188.
